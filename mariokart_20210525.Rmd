...
ggplot() + 
  ...
  geom_star(
    data = filter(rr, shortcut == "No"), 
    aes(x = date, y = time, group=shortcut), 
    color = grey,  size = 2.1, stroke=1.3, fill="white", starshape = 1
    ) +
  geom_line(
  ...
  geom_star(
    data = filter(rr, shortcut == "Yes"), 
    aes(x = date, y = time, color = date, group=shortcut), 
    size = 2.1, stroke=1.3, fill="white", starshape = 1
    ) +
  annotate(
    "label", 
    x = ymd("2019-01-01"), 
    y = max(filter(rr, shortcut == "No", date >= ymd("20170101"))$time) + 25, 
    label = "Without shortcuts", family=font,
  ...
  annotate(
    ...
    label = "With shortcuts", family=font,
    hjust = 1, lineheight = 1, 
    fill = "white", label.size = NA,size = 3.3, color = blue
    ) + 
  annotation_custom(
    lakitu, 
    xmin = ymd("2018-01-01"), xmax = ymd("2021-01-01"), 
    ymin = 390, ymax = 520
    ) + 
  ...
  labs(
    x = "", 
    y = "Time (seconds)\n",
    title = "FINISH! N64 Rainbow Road World Records", 
    subtitle = subtitle
    ) + 
  theme_minimal(base_family = font) + 
  theme(
    legend.position = "none", 
    plot.subtitle = element_text(family = "Garamond", color=grey, size =8), 
    axis.text = element_text(size = 6, color= grey), 
